...
[← Лекция 16](Лекция%2016.%20Конфигурирование%20базы%20данных.md) | [Оглавление](../../README.md) | [Практическая работа 19 →](Практическая%20работа%2019.%20Паттерн%20Репозиторий.md)
...
* Используется команда `dotnet ef dbcontext scaffold`.
* Удобен для работы с уже существующими системами.
* Пример команды генерации моделей из базы данных PostgreSQL:

```bash
dotnet ef dbcontext scaffold "Host=localhost;Port=5432;Database=shopdb;Username=postgres;Password=postgres" Npgsql.EntityFrameworkCore.PostgreSQL -o Models
```
...
* PostgreSQL 16 — `winget install PostgreSQL.PostgreSQL.16` или установщик с сайта [postgresql.org](https://www.postgresql.org/download/windows/);
* инструмент EF Core CLI — `dotnet tool install --global dotnet-ef --version 10.0.12`.
...
8. Как подключить проект к PostgreSQL и SQLite?
