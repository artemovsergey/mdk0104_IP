...
[← Лекция 24](Лекция%2024.%20Типы%20архитектур%20(монолит,%20модульный%20монолит,%20микросервисы).md) | [Оглавление](../../README.md) | [Практическая работа 26 →](Практическая%20работа%2026.%20Разработка%20приложения%20Razor%20Page.md)
...
4. **Развертывание веб-приложений на IIS**
   ...
   * Публикация через `dotnet publish`
...
**Пример публикации через `dotnet publish`:**

```bash
dotnet publish -c Release -o ./publish
```

* Скопировать содержимое папки `./publish` на сервер с IIS
* Указать путь к папке в настройках сайта IIS
...
Начиная с .NET 8 базовый образ `aspnet` по умолчанию слушает порт 8080 (переменная `ASPNETCORE_HTTP_PORTS=8080`), поэтому в Dockerfile указан `EXPOSE 8080`.
...
* Развертывание через GitHub Actions, `dotnet publish` или Azure CLI
