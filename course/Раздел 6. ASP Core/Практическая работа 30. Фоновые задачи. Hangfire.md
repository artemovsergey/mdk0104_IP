...
[← Практическая работа 29](Практическая%20работа%2029.%20Работа%20с%20JWT%20токенами,%20ролями,%20разрешениями.md) | [Оглавление](../../README.md) | [Практическая работа 31 →](Практическая%20работа%2031.%20Валидация.%20FluentValidation.md)
...
2. Добавить пакеты Hangfire через NuGet:

```bash
dotnet add package Hangfire --version 1.8.25
dotnet add package Hangfire.AspNetCore --version 1.8.25
dotnet add package Hangfire.InMemory --version 1.0.0
```

> **Примечание.** Hangfire зависит от `Newtonsoft.Json` 11.x, для которого действует предупреждение об уязвимости (NU1903). Чтобы убрать предупреждение, обновите транзитивную зависимость:
>
> ```bash
...
3. Настроить Hangfire в `Program.cs`:
   ...
   * Подключение хранилища (в учебных примерах — **InMemory**, без внешней БД; в production — PostgreSQL).
