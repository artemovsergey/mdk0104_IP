...
dotnet add package Serilog.Settings.Configuration --version 10.0.1
```

3. Настроить Serilog в `Program.cs`:
...
Чтобы приложение читало эту секцию, подключите её в `Program.cs`:
...
12. Реализовать запись логов в базу данных (например, SQLite или PostgreSQL).
